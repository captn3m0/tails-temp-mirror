...
class Mount(object):
    ...
    def activate(self):
        ...
        if not os.getenv("SYMLINK_ATTACK_TEST"):
            for p in sorted(self.dest.parents) + [self.dest]:
                if p.is_symlink():
        ...
        if not os.getenv("SYMLINK_ATTACK_TEST"):
            if self.is_file and not self.dest.is_file():
                msg = f"Destination {self.dest} exists but is not a regular file"
                raise FailedPrecondition(msg)
            if not self.is_file and not self.dest.is_dir():
                msg = f"Destination {self.dest} exists but is not a directory"
                raise FailedPrecondition(msg)
